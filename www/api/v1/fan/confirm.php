...
/**
 * GET /api/v1/fan/confirm.php?token=xxx
 *
 * Confirms fan club membership via token link from email.
...
$stmt = $db->prepare('SELECT id, status FROM user_fan_club_members WHERE confirm_token = :token');
$stmt->execute([':token' => $token]);
$member = $stmt->fetch();

if (!$member) {
    showPage('Ошибка', 'Заявка не найдена. Возможно, ссылка устарела.', true);
}

if ($member['status'] === 'confirmed') {
    showPage('Уже подтверждено', 'Ваше участие в фан-клубе уже было подтверждено ранее.', false);
}

if ($member['status'] === 'unsubscribed') {
    showPage('Ошибка', 'Это участие было отменено.', true);
}

$stmt = $db->prepare(
    'UPDATE user_fan_club_members SET status = :status, confirmed_at = NOW() WHERE id = :id'
);
$stmt->execute([':status' => 'confirmed', ':id' => $member['id']]);

showPage('Добро пожаловать в фан-клуб!', 'Ваше участие в фан-клубе подтверждено. Спасибо!', false);
