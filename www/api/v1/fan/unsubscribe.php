...
/**
 * GET /api/v1/fan/unsubscribe.php?token=xxx
 *
 * Unsubscribes a fan club member via token link.
...
$stmt = $db->prepare('SELECT id, status FROM user_fan_club_members WHERE unsubscribe_token = :token');
$stmt->execute([':token' => $token]);
$member = $stmt->fetch();

if (!$member) {
    showPage('Ошибка', 'Участие не найдено. Возможно, ссылка устарела.', true);
}

if ($member['status'] === 'unsubscribed') {
    showPage('Уже отписаны', 'Вы уже были отписаны из этого фан-клуба.', false);
}

$stmt = $db->prepare(
    'UPDATE user_fan_club_members SET status = :status WHERE id = :id'
);
$stmt->execute([':status' => 'unsubscribed', ':id' => $member['id']]);

showPage('Вы отписаны', 'Вы успешно покинули фан-клуб. Мы будем рады видеть вас снова!', false);
