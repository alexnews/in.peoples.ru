...
/**
 * GET|POST /api/v1/info-change/admin.php
 *
 * Admin endpoint — list and manage info change requests.
 * Requires admin role.
 *
...
switch ($action) {
    ...
    case 'update_status': {
        ...
        // Get current request
        $existing = $db->prepare('SELECT id, status FROM user_info_change_requests WHERE id = :id');
        ...
        $updateSql = 'UPDATE user_info_change_requests SET status = :status, reviewed_by = :reviewed_by';
        if ($note !== '') {
    ...
    case 'add_note': {
        ...
        $db->prepare('UPDATE user_info_change_requests SET admin_note = :note, reviewed_by = :reviewed_by WHERE id = :id')
